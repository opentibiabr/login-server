...
When TLS terminates at an upstream proxy such as Cloudflare, do not replace the proxy's `X-Forwarded-Proto` with Nginx's local `$scheme`: the proxy-to-origin hop may be HTTP even though the client used HTTPS, which disables trusted-device enrollment. Preserve the upstream value only for authenticated or explicitly allowlisted proxy addresses, and fall back to `$scheme` for direct origin traffic. Never trust a client-supplied forwarding header from an unrestricted source. Cloudflare publishes both its [request-header contract](https://developers.cloudflare.com/fundamentals/reference/http-headers/) and its current [origin-facing IP ranges](https://developers.cloudflare.com/fundamentals/concepts/cloudflare-ip-addresses/).
